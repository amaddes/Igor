	...
	if (req.body.request.command == "no text") {
		...
		if (req.body.request.nlu.tokens[3] == "спальне") {
			mongoClient.connect(url, {
				...
				if (err) return console.log(err);
				collection.find({
					allow_user_id: req.body.session.user_id
				}).toArray(function (err, resultSt) {
					if (resultSt.length > 0) {
						stId = resultSt[0].key;
						console.log(stId);
						devCollection = db.collection("controlDev");
						devCollection.find({
							"stationId": stId,
							"activeFrase": "спальня"
						}).toArray(function (err, resultDev) {
							devId = resultDev[0].devId;
							controlMsg = {
								magic: "&CTRL",
								devId: devId,
								state: "OFF"
							};
							clients[resultSt[0].id_websocket].send(JSON.stringify(controlMsg));
						});
